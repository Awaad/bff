...
def test_project_pagination_index_matches_accepted_order(
    ...
    with postgres_test_db.owner_connection() as connection:
        index_definition = connection.execute(
            ...
            WHERE schemaname = 'app'
              ...
              AND indexname = 'idx_projects_workspace_created_id'
            """
        ).fetchone()
...
def test_project_page_query_can_use_pagination_index(
    postgres_test_db: DatabaseTestEnvironment,
) -> None:
    with postgres_test_db.owner_connection() as connection:
        ...
        explained = connection.execute(
            """
            EXPLAIN (FORMAT JSON)
            SELECT id, workspace_id, name, status, created_at, updated_at, archived_at
            FROM app.projects
    ...
    assert "idx_projects_workspace_created_id" in _index_names(explained[0])
